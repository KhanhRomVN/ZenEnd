...
def _create_plan_mode_response(model: str, messages: List[Any], stream: bool = False) -> dict:
    ...
    """
    plan_response = "<focus_chain id=\"fc-20251121-01\">\n  <task id=\"task-1\" title=\"Refactor WebSocket layer\">Tối ưu hóa và tách WebSocket logic thành service riêng.</task>\n\n  <steps>\n    <step id=\"1\" status=\"todo\" focus=\"true\">\n      <title>Phân tích hiện trạng</title>\n      <instruction>Đọc các file liên quan: ws-manager.js, background.js, socket-controller. Liệt kê coupling và điểm cần tách.</instruction>\n      <output></output>\n    </step>\n\n    <step id=\"2\" status=\"todo\" focus=\"false\">\n      <title>Thiết kế Service API</title>\n      <instruction>Định nghĩa interface cho WebSocketService (connect, send, on, off, reconnect). Viết spec ngắn.</instruction>\n      <output></output>\n    </step>\n\n    <step id=\"3\" status=\"todo\" focus=\"false\">\n      <title>Tách logic</title>\n      <instruction>Di chuyển logic kết nối/queue/reconnect vào WebSocketService, giữ controller nhẹ.</instruction>\n      <output></output>\n    </step>\n\n    <step id=\"4\" status=\"todo\" focus=\"false\">\n      <title>Refactor controllers</title>\n      <instruction>Update background và content scripts để dùng WebSocketService; thêm logging và error handling.</instruction>\n      <output></output>\n    </step>\n\n    <step id=\"5\" status=\"todo\" focus=\"false\">\n      <title>Kiểm thử end-to-end</title>\n      <instruction>Chạy scenario: connect, send/receive, reconnect; ghi lỗi và sửa.</instruction>\n      <output></output>\n    </step>\n  </steps>\n\n  <analysis>Ưu tiên phân tích kỹ file để tránh breaking change; step 1 phải hoàn thành trước khi tách.</analysis>\n  <result></result>\n  <reflection></reflection>\n</focus_chain>"
    if stream:
